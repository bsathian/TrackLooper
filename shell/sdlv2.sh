...
if [ ! -d "${OUTPUTDIR}" ]; then
    ...
    for i in $(seq 0 $((NJOBS-1))); do
        ...
        if [[ ${DOPEREVENT} == true ]]; then
            echo "$TRACKLOOPERBASE/bin/sdl -N ${NMATCH} -x ${i} -i ${SAMPLE} -n 100 -t trackingNtuple/tree -m ${MODENUMBER} -p ${PTBOUND} -o ${OUTPUTDIR}/${OUTPUTFILEBASENAME}_${i}.root -g ${PDGID} -v 3 > ${OUTPUTDIR}/${OUTPUTFILEBASENAME}_${i}.log 2>&1" >> .jobs.txt
        else
